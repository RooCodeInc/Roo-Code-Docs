...
*   **Global Modes:** The migration from `custom_modes.json` to `custom_modes.yaml` happens automatically when Roo Code starts up, under these conditions:
...
*   **Project Modes (`.roomodes`):**
    ...
    *   **Format Detection:** Roo Code can read `.roomodes` files in either YAML or JSON format.
