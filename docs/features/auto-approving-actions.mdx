...
**Description:** "Automatically select the first AI suggested answer for follow-up questions after the configured timeout."

**Timeout slider:**  "Time before the default answer is selected" (Default: 60s)

**Note:** "Will not switch modes unless Mode Switching is enabled"

**Risk level:** Low

When Roo needs clarification, it will ask a follow-up question with 2-4 suggested answers. With this setting enabled, Roo will automatically select its preferred answer after a configurable timeout. This is recommended for autonomous operation to prevent Roo from getting stuck waiting for input.
- Longer time outs give the you more time to read through suggested answers
- Shorter time outs allow Roo to work faster
